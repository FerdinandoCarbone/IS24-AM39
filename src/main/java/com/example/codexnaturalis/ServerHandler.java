...
public class ServerHandler extends Thread implements Runnable {
    ...
    public void broadCastStartingMessageHandler(BroadCastStartingMessage initialMatchSetupMessage) throws IOException {
        try {
            if (initialMatchSetupMessage.equals(null))
                throw new WrongMessageConversionException("Was not able to initialize Starting Field");
    ...
    public void universalStatusUpdater(StandardMatchMessage newStatus) {
        ...
        if (Client.isGuiSelector())
            Platform.runLater(() -> MainController.scoreTracker.moveToken(oldPlayer, newStatus.getCurrPlayerPoints()));
    ...
    public void winnerDeclaration(EndMatchMessage message) throws WrongMessageConversionException {
        ...
        switch (i) {
            ...
            case -1:
                break;
            default:
                throw new WrongMessageConversionException("There was a problem declaring the winner");
        }
        if (Client.isGuiSelector()) {
    ...
    public void restartClient() {
        ...
        try {
            if (Client.isGuiSelector()) {
                ...
                Platform.runLater(() -> {
                    MainController.alert("Server crashed. Client is restarting: at startup, input same username to rejoin match", true);
                    try {
                        Client.restartScene();
                    } catch (IOException e) {
                    ...
                    //Client.getServerHandler().samvise.release();
                });
                //samvise.acquire();
            ...
            if (Client.isGuiSelector()) {
                Platform.runLater(() -> {
                    MainController.alert("Error: Please try manually restart client with same nickname to rejoin match", true);
                });
...
class ServerSocketHandler extends ServerHandler {
    ...
    public void run() {
        while (!Thread.currentThread().isInterrupted()) {
            ...
            try {
                if (socket.isClosed() && Client.isCurrentGameStatus())
                    throw new ClientAbruptlyDisconnectedException(getClientName() + " abruptly disconnected from server: Attempting reconnection");
    ...
    private void messageReceiver(Message inputmex) throws IOException, ClassNotFoundException, WrongMessageConversionException, InterruptedException {
        ...
        switch (messageType) {
            ...
            case "StandardMatchMessage":
                ...
                break;
            default:
                throw new WrongMessageConversionException("Something went wrong while communicating with the server: " + a.getName() + " is not Handled");
        }
    }
